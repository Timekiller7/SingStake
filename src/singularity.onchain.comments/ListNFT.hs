...
-- Build the token name from the signatory's `PPubKeyHash`
  ...
  {- 
    pcon:
      * in docs: Dynamic building of constant `Term`s with `pcon`
        - for ex: to go from `PJust a` to `Term s (PMaybe a)` => 
          - pcon :: a s -> Term s a
  -} 
